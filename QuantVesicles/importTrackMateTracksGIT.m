function [tgoodTrack,tracks, metadata] = importTrackMateTracksGIT(file, clipz, scalet)
...
% Jean-Yves Tinevez <jeanyves.tinevez@gmail.com> - 2013
    ...
    for i = 1 : nTracks
        ...
        tracks{i} = A;
        if (size(tracks{i},1)>25)     
        trackTest1{i} = A;
        end
    end
    %Select relevant cells
     tgoodTrack = reshape(trackTest1(~cellfun(@isempty,trackTest1)),1,[])';
%     for i = 1:size((trackTest),2)
%         if(size(tracks{i},1))~= 0
%             trackTest1{i} = trackTest{i};
%         end
%         
%     end
